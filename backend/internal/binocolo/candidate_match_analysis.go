...
func (s *maService) analyzeCandidateMatch(ctx context.Context, input CandidateMatchAnalysisRequest, subject, email string) (CandidateMatchAnalysisResponse, error) {
	...
	// FinalDecision is intentionally left to the caller: the UC2 job derives it from
	// the concept verdict (sectorFinalDecision); this lab path returns the raw analyst
	// reading. The old magic-threshold reconciliation was demolished.
	return analysis, nil
}
