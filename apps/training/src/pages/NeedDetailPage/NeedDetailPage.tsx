...
import { useDeleteNeed, useNeedDetail, useRemoveNeedCandidate, useRemoveNeedRequest, useSaveNeed } from '../../api/queries';
import type { NeedAcceptResponse, NeedCandidate, NeedStatus } from '../../api/types';
...
import { NeedCandidateModal } from '../../components/needs/NeedCandidateModal';
...
export function NeedDetailPage() {
  ...
  const removeCandidate = useRemoveNeedCandidate();
  const removeRequest = useRemoveNeedRequest();
  const deleteNeed = useDeleteNeed();
  const navigate = useNavigate();
  const [editing, setEditing] = useState(false);
  const [candidate, setCandidate] = useState<NeedCandidate | 'new' | null>(null);
  ...
  async function unlinkCourse(courseId: string) {
    if (!need) return;
    setError(null);
    try { await removeCandidate.mutateAsync({ id: need.id, courseId }); }
    catch (e) { setError(describeApiError(e, 'Rimozione candidato non riuscita')); }
  }
      ...
      {detail.isPending ? <Skeleton rows={6} /> : detail.isError ? (
        ...
        <section className={styles.section} aria-labelledby="need-candidates-title">
          <div className={page.header}>
            <h2 id="need-candidates-title" className={styles.sectionTitle}>Rosa dei corsi</h2>
            <Button variant="secondary" onClick={() => setCandidate('new')}>Aggiungi candidato</Button>
          </div>
          {need.candidates.length > 0 ? <>
            <div className={styles.finalCourse}>
              <p className={styles.muted}>Corso definitivo</p>
              <SingleSelect<string> ariaLabel="Corso definitivo" placeholder="Scegli dalla rosa…" selected={need.finalCourseId ?? null} allowClear clearLabel="Nessun definitivo"
            ...
            {need.candidates.map((c) => (
              <article key={c.courseId} className={styles.candidate}>
                <div className={page.header}>
                  <div className={styles.actions}>
                    ...
                    {c.rank != null && <span className={styles.muted}>Rango {formatNumber(c.rank)}</span>}
                  </div>
                  <div className={styles.actions}>
                    <Button variant="ghost" onClick={() => setCandidate(c)}>Modifica nota e rango</Button>
                    <Button variant="ghost" onClick={() => void unlinkCourse(c.courseId)} disabled={removeCandidate.isPending || c.courseId === need.finalCourseId} title={c.courseId === need.finalCourseId ? 'Cambia o rimuovi prima il definitivo' : undefined}>Rimuovi dalla rosa</Button>
                  </div>
                </div>
          ...
          </> : <p className={styles.muted}>Aggiungi un corso esistente o un candidato da solo titolo.</p>}
        </section>
        <section className={styles.section} aria-labelledby="need-requests-title">
          ...
          {!need.finalCourseId && need.requests.some((r) => !r.outcome) && <p className={styles.muted}>Scegli il corso definitivo nella rosa per accogliere le richieste insieme.</p>}
        ...
        {candidate && <NeedCandidateModal need={need} candidate={candidate === 'new' ? undefined : candidate} onClose={() => setCandidate(null)} />}
