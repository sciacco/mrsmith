...
function StorageDetail({ item, canOperate, onEdit, onArchive }: { item: StorageItem; canOperate: boolean; onEdit: () => void; onArchive: () => void }) {
  return <><div className={styles.header}><div><h2 className={styles.emptyTitle}>{item.size ?? '-'} {item.sizeType ?? ''}</h2><div className={styles.inlineMeta}><StorageEquipmentReference item={item} /><span className={styles.emptyText}>cliente {item.customerId}</span></div></div><span className={item.readOnly ? styles.badgeDanger : styles.badge}>{item.status}</span></div><div className={styles.detailGrid}><Detail label="Protocollo" value={item.protocol} /><Detail label="Apparato" value={<StorageEquipmentReference item={item} />} /><Detail label="Ordine" value={item.orderCode} /><Detail label="Seriale" value={item.serialNumber} /><Detail label="Creato" value={item.createdAt} /><Detail label="Chiuso" value={item.closedAt} /><Detail label="Note" value={item.note} /></div>{item.readOnly ? <p className={styles.emptyText}>Storage chiuso in sola consultazione.</p> : null}{canOperate && !item.readOnly ? <div className={styles.modalActions}><Button variant="secondary" onClick={onEdit}>Modifica</Button><Button variant="danger" onClick={onArchive}>Archivia</Button></div> : null}</>;
}

function StorageEquipmentReference({ item }: { item: StorageItem }) {
  return <span className={styles.equipmentReference}><span>{item.equipment ?? `Apparato ${item.equipmentId}`}</span>{item.equipmentType ? <EquipmentTypeBadge type={item.equipmentType} visual={item.equipmentTypeVisual} compact /> : null}</span>;
}
