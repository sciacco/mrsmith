...
function ServerSummary({ item }: { item: ServerItem }) {
  return <div className={styles.detailGrid}><Detail label="Cliente" value={item.customerId} /><Detail label="Hostname" value={item.hostname} /><Detail label="Ordine" value={item.orderCode} /><Detail label="Seriale" value={item.serialNumber ?? item.serial} /><Detail label="Rack" value={item.rackName} /><Detail label="Apparato" value={<EquipmentReference name={item.equipmentName} type={item.equipmentType} visual={item.equipmentTypeVisual} />} /><Detail label="IP gestione" value={item.managementIp} /><Detail label="Note" value={item.note} /></div>;
}

function EquipmentReference({ name, fallback, type, visual }: { name?: string; fallback?: string; type?: string; visual?: ServerItem['equipmentTypeVisual'] }) {
  if (!name && !fallback) return <>-</>;
  return <span className={styles.equipmentReference}><span>{name ?? fallback}</span>{type ? <EquipmentTypeBadge type={type} visual={visual} compact /> : null}</span>;
}
