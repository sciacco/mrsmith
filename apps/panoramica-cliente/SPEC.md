...
**Interactions:** Click row → panel opens. Click another row → cross-fade. Arrow keys navigate. Escape closes. The active detail dataset can be exported to Excel, preserving filters and native numeric/date values.
...
**Table:** Invoice lines with visual grouping (document header on `rn=1` only, computed in backend SQL). Visible: Documento, Descrizione Riga, Qta, Importo Uni (EUR), Totale Riga (EUR), Codice Articolo, Serial N, Rif Cliente, Pagamento, Scadenza, Conto Ricavo, Gruppo, Sottogruppo. XLSX export is generated by the authenticated backend endpoint.
...
**Table:** 16 columns. Does NOT auto-query on page load with empty selection. Excel export preserves the filtered and sorted dataset.
...
**Master (account list):** Searchable list with accent-bar selection; shows Intestazione + Credito + Abbreviazione. `cloudstack_domain` is the hidden key. The filtered and sorted account list can be exported to Excel.
...
- **Bucket table:** One row per bucket (Periodo, Totale), sortable + Excel export. The exported period uses the same localized bucket label shown in the table.
...
**Layout:** Tenant selector (searchable) → summary (total users, total SE) + PBX table (PBX Name, PBX ID, Users, Service Extensions, Totale). Excel export includes the filtered and sorted PBX rows, not the summary KPIs.
...
| Table features | Search, sort, pagination, column visibility toggle, Excel export |
...
- Tables: Excel export enabled on Ordini dettaglio, Accessi, IaaS account/consumi and Timoo; Fatture uses its dedicated backend XLSX export
