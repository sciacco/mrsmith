...
async function bootstrap() {
  ...
  if (!res.ok) {
    throw new Error(`Coperture auth bootstrap failed with status ${res.status}.`);
  }

  const config: BootstrapConfig = await res.json();
  if (!config.keycloakUrl || !config.realm || !config.clientId) {
    throw new Error('Coperture auth bootstrap is missing Keycloak frontend configuration.');
  }
...
bootstrap().catch((error: unknown) => {
  const message = error instanceof Error ? error.message : 'Coperture auth bootstrap failed.';
