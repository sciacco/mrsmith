...
- **Credential stripping**: no `authentication` block on datasources; connection strings are not present in the export. For the target MrSmith rewrite, GW REST auth is resolved to the existing backend `arak.Client` service-account transport (`ARAK_*` env vars), while DB DSNs remain backend-only runtime config.
