...
async function bootstrap() {
  ...
  if (!res.ok) throw new Error(`Ordini bootstrap failed with status ${res.status}.`);
  ...
  createRoot(document.getElementById('root')!).render(
    <main className="fatal">
      <section>
        ...
        <p>Non è stato possibile aprire Ordini in questo momento. Ricarica la pagina o riapri l&apos;app dal portale.</p>
      </section>
    </main>,
  );
}
...
bootstrap().catch((error: unknown) => {
  console.error('Ordini bootstrap failed.', error);
  renderFatalError();
});
