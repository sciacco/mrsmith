...
async function bootstrap() {
  ...
  if (!res.ok) {
    throw new Error(`RDF auth bootstrap failed with status ${res.status}.`);
  }
  ...
  if (!config.keycloakUrl || !config.realm || !config.clientId) {
    throw new Error('RDF auth bootstrap is missing Keycloak frontend configuration.');
  }
...
bootstrap().catch((error: unknown) => {
  const message = error instanceof Error ? error.message : 'RDF auth bootstrap failed.';
