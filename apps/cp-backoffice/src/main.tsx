...
async function bootstrap() {
  ...
  if (!res.ok) {
    throw new Error(`Avvio non riuscito (stato ${res.status}).`);
  }
  ...
  if (!config.keycloakUrl || !config.realm || !config.clientId) {
    throw new Error('Configurazione di avvio incompleta.');
  }
...
bootstrap().catch((error: unknown) => {
  const message = error instanceof Error ? error.message : 'Avvio non riuscito.';
