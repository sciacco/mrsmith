...
| **Q-A2** | "Invia ordine al fornitore" — actual widget is `btn_sendOrder` (standalone button inside `cnt_fornitore` container, **not** part of `ButtonGroup1` as the audit text suggested). Verified bindings: `isVisible: true`, `isDisabled: state != "PENDING_SEND"`, no role check. **Resolution: 1:1 — state-only gate, no role.** Audit error noted; downstream phases use the verified bindings. |
