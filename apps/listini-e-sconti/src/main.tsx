...
async function bootstrap() {
  ...
  if (!res.ok) {
    throw new Error(`Listini e Sconti auth bootstrap failed with status ${res.status}.`);
  }

  const config: BootstrapConfig = await res.json();
  if (!config.keycloakUrl || !config.realm || !config.clientId) {
    throw new Error('Listini e Sconti auth bootstrap is missing Keycloak frontend configuration.');
  }
...
bootstrap().catch((error: unknown) => {
  const message = error instanceof Error ? error.message : 'Listini e Sconti auth bootstrap failed.';
