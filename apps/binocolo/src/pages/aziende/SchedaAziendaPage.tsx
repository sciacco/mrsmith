...
export function SchedaAziendaPage() {
  ...
  const overviewQuery = useQuery({
    ...
    // I tag sono condivisi tra utenti: riletti a ogni apertura della scheda e
  ...
  // Tipizzati obbligatori nel contratto; il fallback copre solo payload di un
  // backend non ancora aggiornato (mig 148 non applicata). Definito prima dei
  // return anticipati perché usato anche dall'effetto di focus sui tag.
  const companyTags = overview?.tags ?? [];
  ...
  useEffect(() => {
    function onKey(event: KeyboardEvent) {
      ...
      if (excludeOpen || tagsOpen) return;
  ...
  }, [excludeOpen, tagsOpen, scrollToSection, cohortPosition, goToCohortKey]);
