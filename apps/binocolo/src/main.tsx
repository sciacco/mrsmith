...
async function bootstrap() {
  ...
  if (!res.ok) {
    throw new Error(`Binocolo auth bootstrap failed with status ${res.status}.`);
  }
  ...
  if (!config.keycloakUrl || !config.realm || !config.clientId) {
    throw new Error('Binocolo auth bootstrap is missing Keycloak frontend configuration.');
  }
...
bootstrap().catch((error: unknown) => {
  const message = error instanceof Error ? error.message : 'Binocolo auth bootstrap failed.';
