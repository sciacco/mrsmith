...
- Il brief (scorecard, valutazione, testo) si consulta dal drawer della card e dal **MA card-dossier** (`/iniziative/:id/dossier/:companyKey`), che il drawer apre con «Apri dossier» e che legge la cache deep company-keyed. Il tool standalone `/azienda` è indipendente da MA e **non è accoppiato** al flusso (ratifica D-B, 2026-07-02). **Emendamento EA-1 (2026-07-05)**: il piano esecutivo (`INIZIATIVE-IMPLEMENTATION-PLAN.md`, F2/F4) nella sua stesura originale puntava erroneamente i link "Apri dossier"/"Gestione dal dossier" a `/azienda?vat=` e colloccava il registro in `CompanyDossierPage`; è stato emendato. `/azienda` resta esclusivamente quick review standalone via P.IVA; il dossier di lavorazione (deep + registro + tesi + IRL) vive sul card-dossier, dove l'implementazione reale lo ha già collocato.
