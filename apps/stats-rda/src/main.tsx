...
async function bootstrap() {
  ...
  if (!res.ok) throw new Error(`Statistiche RDA bootstrap failed with status ${res.status}.`);
  ...
  createRoot(document.getElementById('root')!).render(
    <main className="fatal">
      <section>
        ...
        <p>Non è stato possibile aprire Statistiche RDA in questo momento. Ricarica la pagina o riapri l&apos;app dal portale.</p>
      </section>
    </main>,
  );
}
...
bootstrap().catch((error: unknown) => {
  console.error('Statistiche RDA bootstrap failed.', error);
  renderFatalError();
});
