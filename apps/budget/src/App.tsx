...
export function App() {
  ...
  if (status === 'reauthenticating') {
    return (
      <AppShell appName="Budget Management" userName={user?.name ?? 'John Doe'} onLogout={logout}>
        <AppShell.Nav>
  ...
  if (!authenticated) {
    return (
      <AppShell appName="Budget Management" userName={user?.name ?? 'MrSmith'} onLogout={logout}>
        <AppShell.Nav>
          <TabNav items={navItems} />
        </AppShell.Nav>
        <AppShell.Content>
