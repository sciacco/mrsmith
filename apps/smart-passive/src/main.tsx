...
async function bootstrap() {
  ...
  if (!res.ok) {
    throw new Error(`Smart Passive auth bootstrap failed with status ${res.status}.`);
  }

  const config: RuntimeConfig = await res.json();
  if (!config.keycloakUrl || !config.realm || !config.clientId) {
    throw new Error('Smart Passive auth bootstrap is missing Keycloak frontend configuration.');
  }
...
bootstrap().catch((error: unknown) => {
  const message = error instanceof Error ? error.message : 'Smart Passive auth bootstrap failed.';
