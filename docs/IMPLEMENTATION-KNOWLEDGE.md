...
- Practical rule: initiative-scoped jobs without a search session use nullable `ma_job.session_id`/`strategy_version_id`, set `initiative_id`, and must implement their own active-job deduplication because PostgreSQL unique indexes treat NULL session keys as distinct. `card_domain_verify` is the reference implementation.
