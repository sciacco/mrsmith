...
The "RICHIEDI ANNULLAMENTO" button on `Dettaglio ordine` is **not** ported in v1. The Appsmith binding is doubly out of date versus the canonical contract in `docs/mistra-dist.yaml:1819`: the URL-template param name (`order_Id`) mismatches the handler argument (`order_number`), and the Mistra NG spec requires a `{customer_name: string}` body that the Appsmith action does not send. It is therefore unclear whether the feature currently works in production at all.

When re-enabling post-v1:
- Add backend endpoint `POST /api/ordini/v1/orders/:id/cancel-request` (auth `app_customer_relations`, state must be `INVIATO`). The backend loads the order by vodka `:id`, then calls Mistra NG `POST /orders/v2/order/{order_number}/cancel` with the derived `order_number` and `customer_name` in the body.
- Resolve the two open sub-questions before wiring: (a) which order field maps to `order_number` in the Mistra path — vodka `orders.id`, `cdlan_systemodv`, or `cdlan_ndoc`/`cdlan_anno`; (b) where `customer_name` comes from (most likely `orders.cdlan_cliente`, the persisted RAGIONE_SOCIALE string).
