...
- Add backend endpoint `POST /api/ordini/:id/cancel-request` (auth `app_customer_relations`, state must be `INVIATO`). The backend loads the order by vodka `:id`, then calls Mistra NG `POST /orders/v2/order/{order_number}/cancel` with the derived `order_number` and `customer_name` in the body.
